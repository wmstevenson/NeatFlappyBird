...
class Pipe:
    ...
    def set_height(self):
        self.top_pipe_bottom = random.randint(
            self.MARGIN,
            self.window_height - self.GAP - self.MARGIN
        )
    ...
    def draw(self, window):
        pygame.draw.rect(
            window,
            self.color,
            (self.x, 0, self.WIDTH, self.top_pipe_bottom)
        )

        pygame.draw.rect(
            window,
            (0, 200, 0),
            (self.x, self.bottom_pipe_top, self.WIDTH, self.window_height - self.bottom_pipe_top)
        )
